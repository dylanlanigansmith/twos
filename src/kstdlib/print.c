...
/*
unfortunately we are using this really shitty printf i wrote a while ago 

to future you: just stay out of this file please 
it is too shit to touch 
too shit to look at 
good enough to use blindly
*/
...
#define __PRINT_LLTOA(num, base, buf) ltoa(num, buf, base)
#define __PRINT_ITOA(num, base, buf) itoa(num, buf, base)
#define __PRINT_HTOA(num, buf) ltoa(num, buf, 16)
size_t __vprintf(__printf_out_fn _print, const char *fmt, char *buf_out, size_t len, va_list args)
{
    
    ASSERT((uintptr_t)_print != 0); 
    // should be an arg
